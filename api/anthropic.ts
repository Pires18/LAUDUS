...
export default async function handler(req: Request) {
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, x-api-key, x-uid, anthropic-version, anthropic-beta',
      },
    });
  }
  ...
  try {
    ...
    // Prefer server-side key so the client never needs to expose it.
    // Fall back to client-provided header for self-hosted / dev setups.
    const apiKey = (process.env.ANTHROPIC_API_KEY || '').trim() || req.headers.get('x-api-key') || '';
    if (!apiKey) {
