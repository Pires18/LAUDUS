...
O modelo de dados é histórico **per-usuário** (`users/{uid}/{collection}/{doc}`) — cada usuário é dono de sua própria subárvore de pacientes/exames/clínicas/agenda.

**O que já funciona:** convite de um segundo usuário para colaborar numa clínica (`ClinicTeamCard.tsx` na tela da clínica, papel `editor` ou `viewer`), via `api/clinic-invite.ts` (resolve e-mail→uid, valida posse da clínica). As regras do Firestore já reconhecem `clinic_memberships` e permitiriam o acesso a `clinics`/`patients`/`exams`/`appointments` da clínica compartilhada, incluindo proteção contra reatribuição indevida de `clinicId`.

**O que NÃO funciona ainda:** a camada de dados do cliente (`useFirestore.ts`, `store/db.ts`) sempre consulta `users/{auth.currentUser.uid}/...` — isto é, mesmo com o convite aceito, **o membro convidado não visualiza os dados do dono pela interface atual**. Não anunciar "equipe multiusuário" como pronta para uso real até essa segunda parte (apontar as queries para a subárvore do `ownerId` quando há membership ativo) ser implementada — é uma mudança de maior escopo, toca praticamente todos os módulos, e deve ser feita e testada (idealmente com o emulador do Firestore) numa sessão dedicada.
...
|---|---|---|
| Wiring multi-owner de `clinic_memberships` | Cliente não busca dados da clínica compartilhada, só a própria subárvore | Trabalho técnico, sem decisão externa pendente |
