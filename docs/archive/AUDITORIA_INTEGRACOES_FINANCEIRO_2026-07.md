...
| **Provisionamento de VM (GCP + Tailscale)** | ⚠️ Parcial | Criação de VM implementada e funcional (`api/pacs-provision.ts`), com modo mock seguro quando faltam credenciais. ~~**Falta**: lifecycle completo (suspender/destruir VM ao cancelar assinatura) — mencionado no roadmap como "Fase 6", não implementado.~~ **[CORREÇÃO 09/07/2026: lifecycle completo já implementado em `api/reset-monthly-reports.ts` — suspende/reativa/destrói a VM conforme a assinatura; ver `docs/pacs/PACS_PROVISION_SETUP.md`, item F6 marcado ✅ feito]** |
