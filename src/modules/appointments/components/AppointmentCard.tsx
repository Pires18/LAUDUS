  ...
  return (
    <motion.div
      ...
      className={classNames(
        ...
        appointment.status === 'cancelado' ? "border-rose-200 bg-rose-50/5" : "border-slate-200"
      )}
    >
      ...
      <div className="pl-3 space-y-3">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
            <span className="flex items-center gap-1 bg-slate-100 text-slate-700 px-2.5 py-1 rounded-lg font-bold text-[10px] uppercase tracking-wide">
              <Clock size={12} /> {appTime}
            </span>
            <span className="font-bold text-slate-600">{appDateStr}</span>
            {clinic && (
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5 flex-wrap">
              <h3 className="font-black text-slate-900 text-sm leading-tight truncate max-w-[180px] sm:max-w-[220px]" title={appointment.patientName}>
                {appointment.patientName}
              </h3>
        ...
        <div className="flex items-center gap-2 text-xs font-semibold text-slate-700 bg-slate-50 border border-slate-200/60 p-2.5 rounded-xl">
          <AreaIcon area={appointment.area} size={14} className="text-slate-400 shrink-0" />
        ...
        <div className="grid grid-cols-2 gap-2 text-[10px] text-slate-600 border-t border-slate-100 pt-2.5">
          ...
          {appointment.patientCPF && (
            <div className="flex items-center gap-1 truncate" title={`CPF: ${appointment.patientCPF}`}>
              <span className="font-bold text-slate-400 shrink-0">CPF:</span>
          ...
          {appointment.patientInsurance && (
            <div className="flex items-center gap-1 truncate" title={`Convênio: ${appointment.patientInsurance}`}>
              <span className="font-bold text-slate-400 shrink-0">Conv:</span>
              <span className="truncate font-black text-slate-700">{appointment.patientInsurance}</span>
            </div>
          )}
          {appointment.requestingPhysician && (
            <div className="flex items-center gap-1 truncate" title={`Médico Solicitante: ${appointment.requestingPhysician}`}>
              <span className="font-bold text-slate-400 shrink-0">Dr(a):</span>
        ...
        {/* Notes */}
        {appointment.notes && (
          <p className="text-[10px] text-slate-500 italic bg-slate-50/50 p-2 rounded-xl border border-slate-100/60 truncate" title={appointment.notes}>
      ...
      <div className="flex gap-2 mt-4 pt-3 border-t border-slate-100 pl-3 shrink-0">
        {appointment.status === 'agendado' && (
          <>
            ...
            <button
              ...
              className="p-2 text-slate-600 hover:text-brand-700 hover:bg-slate-50 border border-slate-200 rounded-xl transition-all active:scale-95 flex items-center justify-center shadow-sm"
              title="Reagendar"
            >
              <RefreshCw size={13} />
            </button>
