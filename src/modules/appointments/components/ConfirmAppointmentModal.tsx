...
export function ConfirmAppointmentModal({
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300 animate-in fade-in">
      <motion.div 
      ...
      >
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-2.5 bg-slate-50 p-4 rounded-2xl border border-slate-200/60 text-xs text-slate-700">
            <div>
              <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Paciente</span>
              <p className="font-bold text-slate-900">{appointment.patientName}</p>
            </div>
            <div>
              <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Exame / Procedimento</span>
              <p className="font-bold text-slate-900">{appointment.examType}</p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Horário</span>
                <p className="font-bold text-slate-900">{appTime}</p>
              </div>
              <div>
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Data</span>
                <p className="font-bold text-slate-900">{appDateStr}</p>
              </div>
            </div>
          </div>
          ...
          {dicomDevices && dicomDevices.length > 0 && (
            <div className="space-y-1.5">
              <label className="text-[9px] font-black text-slate-550 uppercase tracking-widest block ml-0.5">Dispositivo PACS</label>
              <select
                value={selectedDeviceId}
                onChange={(e) => setSelectedDeviceId(e.target.value)}
                className="w-full h-11 px-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-xs text-slate-900 focus:border-slate-400 outline-none cursor-pointer"
              >
