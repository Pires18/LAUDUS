  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-ink-900/40 backdrop-blur-sm animate-in fade-in duration-300">
      <motion.div 
        ...
        className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden flex flex-col max-h-[85vh] modal-mobile-sheet"
      >
        {/* Header */}
        <div className="px-6 py-5 border-b border-ink-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-ink-900 leading-tight">Reagendar Atendimento</h3>
          ...
          <button 
            type="button" 
            onClick={onClose} 
            className="p-2 text-ink-400 hover:text-ink-700 hover:bg-ink-50 rounded-xl"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
          {/* Exam info */}
          <div className="bg-ink-50 p-4 rounded-2xl border border-ink-200 shadow-sm text-xs">
            <span className="text-[8px] font-black text-ink-400 uppercase tracking-widest block mb-0.5">Exame selecionado</span>
            <p className="font-bold text-ink-900">{appointment.examType}</p>
          </div>

          {/* New Date */}
          <div className="space-y-1.5">
            <label className="text-[9px] font-black text-ink-500 uppercase tracking-widest block ml-0.5">Nova Data</label>
            <input
              type="date"
              value={newDate}
          ...
          <div className="space-y-2">
            <label className="text-[9px] font-black text-ink-500 uppercase tracking-widest block ml-0.5">Horários Disponíveis</label>
          ...
          {/* Notes */}
          <div className="space-y-1.5">
            <label className="text-[9px] font-black text-ink-500 uppercase tracking-widest block ml-0.5">Observações / Indicação Clínica</label>
            <textarea
        ...
        {/* Footer */}
        <div className="px-6 py-5 border-t border-ink-100 bg-white flex items-center justify-between shrink-0">
          <button
            type="button"
            onClick={onClose}
