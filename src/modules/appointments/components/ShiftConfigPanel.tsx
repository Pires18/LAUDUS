...
export function ShiftConfigPanel({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm space-y-8 max-w-5xl mx-auto">
      <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
        <Sliders className="text-slate-800" size={22} />
        <div>
          <h3 className="font-black text-slate-900 text-lg leading-tight">Configurações de Agenda & Turnos</h3>
          <p className="text-xs text-slate-500">Defina múltiplos turnos de trabalho (Manhã, Tarde, Noite...) por dia e parametrize a duração do exame de cada um.</p>
        </div>
      </div>

      <div>
        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Unidade / Clínica</label>
        <select
          value={configClinicId}
          onChange={(e) => onClinicChange(e.target.value)}
          className="w-full max-w-sm h-12 px-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-xs text-slate-900 focus:border-slate-400 outline-none cursor-pointer"
        >
          {clinics.map(c => (
              ...
              <div className="flex items-center justify-between pb-3 border-b border-slate-200">
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="w-4 h-4 rounded text-slate-900 focus:ring-slate-900/20 cursor-pointer"
                  />
              ...
              {bh.active ? (
                bh.shifts.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {bh.shifts.map((shift) => (
                      <div key={shift.id} className="p-4 rounded-xl bg-white border border-slate-200/60 shadow-sm space-y-3 relative group">
                        <button
                          type="button"
                          onClick={() => handleRemoveShift(bh.day, shift.id)}
                          className="absolute top-3 right-3 p-1.5 text-slate-400 hover:text-rose-600 rounded-lg hover:bg-rose-50 transition-all opacity-0 group-hover:opacity-100"
                          title="Excluir Turno"
                        >
                          <Trash2 size={14} />
                        </button>
      ...
      <div className="flex gap-3 pt-6 border-t border-slate-100 justify-end">
        <button
          ...
          onClick={handleSave}
          disabled={loading}
          className="h-14 px-8 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-sm active:scale-95"
        >
        ...
        <button
          type="button"
          onClick={onCancel}
          className="h-14 px-6 border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-2xl font-bold text-xs uppercase tracking-wider transition-all"
        >
          Voltar
        </button>
      </div>
