  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 lg:p-10">
      ...
      <motion.div 
        ...
        className="relative bg-white/95 backdrop-blur-xl w-full max-w-6xl h-full max-h-[850px] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col border border-slate-100 shadow-slate-950/20"
      >
        {/* Header with Premium Mesh Gradient */}
        <div className="relative h-28 shrink-0 overflow-hidden bg-slate-900 border-b border-slate-800">
          ...
          <div className="relative h-full flex items-center justify-between px-10">
            <div className="flex items-center gap-6">
              <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shadow-2xl backdrop-blur-xl">
              ...
              <div>
                 <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="bg-brand-500/20 text-brand-300 text-[10px] font-black px-2 py-0.5 rounded-lg border border-brand-500/30 uppercase tracking-widest">v2.0</span>
                 </div>
                 <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em]">Módulos de Suporte à Decisão Diagnóstica</p>
              </div>
            </div>
            
            <button 
              onClick={onClose}
              className="w-12 h-12 rounded-2xl bg-white/5 hover:bg-white/10 text-white flex items-center justify-center transition-all border border-white/10 active:scale-95 shadow-inner"
            >
              <X size={24} />
            </button>
          </div>
        ...
        <div className="flex-1 flex min-h-0 relative">
          {!selectedCalcId ? (
            <div className="flex-1 flex flex-col min-h-0 bg-slate-50/50">
              {/* Filter Bar */}
              <div className="px-10 py-5 bg-white border-b border-slate-100 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                     <Filter size={16} className="text-brand-600" />
                     <span className="text-[11px] font-black text-slate-900 uppercase tracking-widest">Filtragem Ativa:</span>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              <div className="flex-1 overflow-y-auto p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 custom-scrollbar">
                {filteredCalculators.map(calc => (
                  <motion.button
                    ...
                    className="group flex flex-col text-left p-8 rounded-[2rem] border border-slate-100 bg-white hover:border-brand-500 hover:shadow-xl hover:shadow-brand-500/5 transition-all relative overflow-hidden shadow-sm"
                  >
                    <div className="w-14 h-14 rounded-2xl bg-slate-50 text-slate-500 flex items-center justify-center mb-6 group-hover:bg-brand-600 group-hover:text-white transition-all shadow-inner border border-slate-100">
                      <AreaIcon area={area || calc.areas[0]} size={28} />
                    </div>
                    <h4 className="font-black text-slate-800 text-lg mb-2 group-hover:text-brand-700 transition-colors uppercase tracking-tight">{calc.name}</h4>
            ...
            <div className="flex-1 flex flex-col min-h-0 bg-slate-50/50">
              ...
              <div className="px-10 py-5 bg-white border-b border-slate-100 flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-3">
                   ...
                   <span className="text-[11px] font-black text-slate-800 uppercase tracking-widest">{selectedCalc?.name}</span>
                </div>
                <div className="w-24" />
              </div>
              ...
              <div className="flex-1 flex min-h-0">
                <div className="flex-1 overflow-y-auto p-10 custom-scrollbar">
                  <div className="max-w-3xl mx-auto bg-white rounded-[2rem] border border-slate-100 shadow-sm p-10">
                    {selectedCalc && (
                      <>
                ...
                <div className="w-[400px] border-l border-slate-100 bg-white flex flex-col shrink-0">
                   <div className="p-8 border-b border-slate-100 bg-slate-50/40">
                      ...
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tight">Resultado sincronizado em tempo real</p>
                   </div>
                      ...
                      <AnimatePresence mode="wait">
                        {calcResult?._summary ? (
                          <motion.div 
                            ...
                            className="space-y-4"
                          >
                             <div className="p-6 rounded-[1.8rem] bg-brand-50/50 border border-brand-100 shadow-inner">
                             ...
                             <div className="space-y-3">
                                ...
                                <div className="space-y-2">
                                   {Object.entries(calcResult).filter(([k]) => !k.startsWith('_')).map(([k, v]) => (
                                     <div key={k} className="flex items-center justify-between px-4 py-2 bg-white rounded-xl border border-slate-100 shadow-sm">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-tight">{k}</span>
                                        <span className="text-[11px] font-black text-slate-700">{String(v)}</span>
                                     </div>
                                   ))}
                                </div>
                             </div>
                          ...
                          <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-40 py-20">
                             ...
                             <p className="text-[11px] font-black text-slate-400 uppercase leading-relaxed max-w-[200px]">Preencha os dados do módulo para gerar a conclusão técnica.</p>
                          </div>
                        )}
                      </AnimatePresence>
                   </div>
                   ...
                   <div className="p-8 border-t border-slate-100 bg-slate-50/40">
                      <button
                        ...
                        className="w-full h-16 rounded-[1.5rem] bg-gradient-to-r from-brand-600 to-brand-700 text-white font-black text-[11px] uppercase tracking-[0.2em] shadow-xl shadow-brand-500/25 hover:from-brand-700 hover:to-brand-800 disabled:opacity-30 disabled:grayscale disabled:shadow-none transition-all flex items-center justify-center gap-3 group active:scale-95"
                      >
