...
export function Preeclampsia2tRiskCalculator({ value, onChange }: CalculatorProps) {
  ...
  const validated = SECOND_TRIMESTER_PE_VALIDATED;
  ...
  }, [canCompute, inWindow, age, weightKg, heightCm, gaWeeksNum, racialOrigin, conception, parity, previousPeGaWeeks, diabetes, chronicHypertension, sleOrAps, familyHistoryPE, smoker, analyzer, mapMmHg, effUta, plgfRaw]);
  ...
  useEffect(() => {
    const base = {
      ...
      mapMmHg, utaR, utaL, plgfRaw,
    };
    if (!result) { onChange({ ...base, _summary: null }); return; }
    const fmt = formatOneInN;
    const disclaimer = validated
      ? 'Apoio à decisão (não é a calc oficial da FMF). '
      : '⚠️ EM VALIDAÇÃO (não usar clinicamente) — ';
    const conduta = result.aspirinRecommended
      ? 'Risco elevado de PE pré-termo — considerar AAS 150 mg/noite conforme protocolo.'
    ...
    onChange({
      ...
      _summary:
        `${disclaimer}Risco de pré-eclâmpsia (2º trimestre, 19–24+6 sem) — basal pré-termo ${fmt(result.basalPretermPE.oneInN)}; ` +
        `corrigido: pré-termo (<37s) ${fmt(result.pretermPE.oneInN)}, a termo ${fmt(result.termPE.oneInN)}. ${conduta}${underflowNote}`,
    });
  }, [result, mapMoM, utaPiMoM, plgfMoM, validated, age, weightKg, heightCm, gaWeeks, racialOrigin, conception, parity, previousPeGaWeeks, diabetes, chronicHypertension, sleOrAps, familyHistoryPE, smoker, analyzer, mapMmHg, utaR, utaL, effUta, plgfRaw, onChange]);
    ...
    <div className="space-y-7">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[10px] text-ink-400 font-bold uppercase tracking-tighter">Rastreamento de 2ª visita (19–24+6 sem) — riscos competitivos</p>
        </div>
      </div>
      ...
      {!validated ? (
        <div className="p-3.5 rounded-xl bg-amber-50 border-2 border-amber-200 flex gap-3 items-start">
          ...
          <div className="text-[11px] text-amber-800 font-bold leading-relaxed">
            EM VALIDAÇÃO (para teste) — fatores maternos por Wright 2015; biomarcadores por Gallo 2016;
            medianas por Tayyar/Wright A/Tsiakkas 2015. Ainda NÃO conferida caso-a-caso contra a
            calculadora oficial da FMF (2ª visita). Não usar para decisão clínica.
          </div>
        </div>
      ) : (
      ...
      <div className="space-y-4">
        ...
        <p className="text-[9px] text-ink-400 font-medium ml-1">
          MoM automático pela IG e covariáveis — medianas de 2º trimestre (Tayyar/Wright A/Tsiakkas 2015). A
          artéria oftálmica NÃO entra no modelo quantitativo de 2º T (permanece marcador qualitativo no laudo).
        </p>
      </div>
          ...
          <PeJourneyCard
            ...
            recommendation={
              result.aspirinRecommended
                ? `Acima do cutoff (1:${DEFAULT_PE_THRESHOLDS.aspirinCutoffOneInN}) — considerar AAS conforme protocolo.`
          ...
          {result.aspirinRecommended && (
            <div className="p-3 rounded-xl bg-rose-50 border border-rose-200 flex gap-3 items-center">
              ...
              <span className="text-[11px] font-bold text-rose-800 leading-relaxed">
                Rastreio de 2ª visita positivo — a profilaxia com AAS é mais eficaz iniciada antes de 16
                semanas; a conduta é sempre clínica.
              </span>
            </div>
          )}
        </div>
