...
export function SharedLaudIA({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const renderPlayground = () => (
    <div className="bg-zinc-950 rounded-2xl border border-zinc-800 overflow-hidden mt-6">
      ...
      {showPlayground && (
        <PlaygroundPanel
          templateSubTab={templateSubTab}
          selectedAreaFilter={selectedAreaFilter}
          templates={templates}
          selectedTemplateId={selectedTemplateId}
          setSelectedTemplateId={setSelectedTemplateId}
          notes={playgroundNotes}
          setNotes={setPlaygroundNotes}
          onRun={handleRunPlaygroundTest}
          isTesting={isPlayinggroundTesting}
          result={playgroundResult}
          score={playgroundScore}
          scratchpad={playgroundScratchpad}
        />
      )}
    </div>
  );
